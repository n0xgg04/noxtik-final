...
const InputField = React.memo(
  React.forwardRef<TextInput, Props>(function (
    ...
    const Icon = React.useMemo(
      () =>
        leftIcon &&
        leftIcon(!isFocus ? colors.colors.no_active : colors.colors.active),
      [isFocus],
    );
      ...
      >
        ...
        {props.secureTextEntry && (
          <Pressable onPress={handleChangeSecureStatus}>
            {isSecureText ? (
              <AntDesign name="eyeo" size={hp(2.8)} color="gray" />
            ) : (
              <Feather name="eye-off" size={hp(2.6)} color="gray" />
            )}
          </Pressable>
        )}
      </Stack>
